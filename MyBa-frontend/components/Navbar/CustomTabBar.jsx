...
function CustomTabBar() {
  ...
  const TabButton = ({ icon: Icon, label, onPress, isActive }) => (
    <TouchableOpacity
      className={`flex-1 items-center justify-center py-3 px-2 ${isActive ? "bg-blue-50" : ""}`}
      onPress={onPress}
      style={{
    ...
    >
      <View className={`p-2 rounded-full ${isActive ? "bg-blue-600" : ""}`}>
        <Icon size={22} color={isActive ? "#ffffff" : "#64748b"} />
      </View>
      <Text className={`text-xs mt-1 font-medium ${isActive ? "text-blue-600" : "text-slate-500"}`}>{label}</Text>
    </TouchableOpacity>
  )
  ...
  return (
    <View className="bg-white border-t border-gray-200 shadow-lg">
      ...
      <View className="flex-row justify-between items-center px-2 py-2">
        ...
        <TabButton icon={Menu} label="Menu" onPress={() => handleTabPress("Menu")} isActive={activeTab === "Menu"} />
      </View>
